        ...
        var defaults = {
            ...
            templates:          {
                ...
                listItem:         '<li class="mentiony-item" data-item-id="">' +
                                  ...
                                  '</div>' +
                                  '<div class="pl0 col-xs-10 col-sm- col-md-10 col-lg-10">' +
                                  '<p class="title">Company name</p>' +
                                  '</div>' +
                                  '</div>' +
                                  '</li>',
                normalText:       '<span class="normalText">&nbsp</span>',
                highlight:        '<span contenteditable="false" class="highlight"></span>',
                image:            '<img src="[SRC]" id="[ID-IMG]" draggable="false" style="border-radius: 40%;background-repeat: no-repeat;background-position: left;margin-bottom:2px;width:13px;height:13px;"/>',
                highlightContent: '<a href="[HREF]" data-item-id="[ITEM_ID]" contenteditable="false" draggable="false" class="highlight disableselect">[TEXT]</a>',
                                  // '<a id="cros-id-[ID]" bypassrouter= "true" contenteditable="false" draggable="false" style="padding-left:2px; padding-right:2px;margin:0px;font-size: 12px;" class="highlight disableselect">&times</a>',
            }
        };
    ...
    var MentionsInput = function (settings) {
        ...
        function initTextArea(ele) {
           ...
           // console.log(elmInputBox);
            ...
            // Container
            //console.log(elmInputBox.attr('id'));
            elmInputBoxContainer = $(settings.templates.container.replace('[ID]', elmInputBox.attr('id')));
            elmInputBoxContent = $(settings.templates.content.replace('[ID]', inputId+'-'+elmInputBox.attr('id')));
        ...
        function updateMentionKeyword(e) {
            ...
            getMentionKeyword();
            //console.log(currentMention.keyword);
        }
        ...
        function setSelectedMention(item) {
            ...
            // console.log(item, 'setSelectedMention item: ');
        }
        ...
        function choseMentionOptions(chooseByMouse) {
            ...
            for (var i = 0, len = currentMention.mentionItemDataSet.length; i < len; i++) {
                if (selectedId == currentMention.mentionItemDataSet[i].id) {
                    ...
                    //console.log(currentMentionItemData.avatar);
                    break;
                }
            }

            var highlightNode = $(settings.templates.highlight);
            var image = $(settings.templates.image
                .replace('[SRC]',currentMentionItemData.avatar)
                .replace('[ID-IMG]',currentMentionItemData.id));
        ...
        function replaceTextInRange(fromString, toTextHtml, choosedByMouse) {
            ...
            //console.log(node.next().text().length);
