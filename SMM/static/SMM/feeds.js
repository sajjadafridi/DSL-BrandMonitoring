...
app.controller('FeedsController', ["$scope", "$http", "$interval",function ($scope, $http, $interval) {
 ...
 $interval( function(){ $scope.resetAlertBadges(); }, 3000);
...
//  var element=$("#feed_"+feed_no);
  var element=angular.element(document.querySelector('#scroll'));
     ...
     $('#scroll').animate({ scrollTop: element[0].offsetTop}, 200);
  };
...
$(document).ready(function() {
 ...
 $('#scroll').css({'height': $(window).height()-$('#dashboardNav').outerHeight() -10 + 'px'});
 $('#scroll1').css({'height': $(window).height()-$('#dashboardNav').height()-100+ 'px'});
